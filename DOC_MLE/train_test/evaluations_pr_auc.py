...
def roc_pr_curve(y_true, y_scores, path_experiment):
    ...
    np.save('../DRIVE/fpr.npy', fpr)
    np.save('../DRIVE/tpr.npy', tpr)
    AUC_ROC = auc(fpr, tpr)
    ...
    np.save('../DRIVE/precision.npy', precision)
    np.save('../DRIVE/recall.npy', recall)
...
def misc_measures(true_vessel_arr, pred_vessel_arr, need_threshold = False):
    ...
    try:
        ...
        # F1_score_2 = 1. * (2. * precision * sensitivity )/ (precision + sensitivity)
        F1_score_2 = 1. * (2. * cm[1, 1]) / (np.sum(cm) + cm[1, 1] - cm[0, 0])
