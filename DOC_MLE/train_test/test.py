...
from DOC_MLE.data_ultils import read_all_images
from DOC_MLE.data_load import ImageFolder,get_drive_data
...
def val_vessel(net1, imgs, masks, length, epoch =0, ch = Constants.BINARY_CLASS):
    ...
    with torch.no_grad():
        ...
        with open(os.path.join('/root/root/Pycharm_Project/DOC_MLE/log', 'save_result.txt'), 'a', encoding='utf-8') as f:
            f.write(s)
            f.write('\n')
...
######################################################threshold-learning#######################################################
    ...
    return new_image.to(device), new_image2.to(device)
